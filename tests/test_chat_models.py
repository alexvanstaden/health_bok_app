...
Every Claude/GPT-backed adapter (Summarizer, Extractor, QueryAnswerer, StanceJudge)
talks to its provider only through `ChatModel.complete(...)`. Two transport
adapters implement it — OpenAI and Anthropic — and a factory picks one from
`LLM_PROVIDER`. These guard the transport shape (how each SDK is called and its
response unwrapped) and the provider selection, with both SDK clients faked.
"""
