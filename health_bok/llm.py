...
One place decides which LLM provider backs every Claude/GPT-powered adapter. The
feature adapters depend only on the `ChatModel` port; this factory reads
`LLM_PROVIDER` and constructs the matching transport with the right key, so
swapping providers — or dropping one to cut an external dependency — never touches
the Summarizer, Extractor, QueryAnswerer, or StanceJudge.
"""
