...
Five tasks call a chat LLM: summarization (ADR-0007), precision-first extraction (ADR-0010),
grounded query synthesis (ADR-0011), the Impact StanceJudge (issue #18), and the Goal
ConceptProposer (issue #39). Each adapter constructed its own Anthropic client and called the
Messages API directly, so the provider was welded into five places and the worker, API, and
pipeline all required `ANTHROPIC_API_KEY`. We want to run on **one** external LLM provider —
OpenAI already does Whisper transcription and Concept embeddings (ADR-0008) — without losing the
ability to swap.
...
- **The feature adapters keep their prompts and parsing** and depend only on an injected
  `ChatModel`. The provider-specific code is *only* the two transports; prompt-building and the
  JSON contracts (`parse_extraction`, `parse_stance`, `parse_answer`, `parse_concepts`) stay
...
- **Per-task model knobs stay** (`SUMMARY_MODEL`, `EXTRACTION_MODEL`, `QUERY_MODEL`,
  `STANCE_MODEL`, `CONCEPT_PROPOSAL_MODEL`), each defaulting to the provider's default chat
  model (OpenAI → `gpt-4.1`), so a task can use a stronger or cheaper model without changing
  provider. The provider-named `CLAUDE_MODEL` knob is renamed `SUMMARY_MODEL`.
...
- **A heavier provider abstraction** (streaming, tool-use, token accounting) — rejected as
  premature: all five tasks are single-turn prompt→text(→JSON). The minimal `complete` seam is
