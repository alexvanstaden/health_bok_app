...
# Which LLM provider backs the four chat tasks — summarize, extract, answer, judge
# (ADR-0012). Defaults to OpenAI so the whole system runs on one external LLM
# provider (embeddings and Whisper already use OPENAI_API_KEY); `LLM_PROVIDER=
# anthropic` swaps in Claude instead. Selection lives in `health_bok.llm`.
DEFAULT_LLM_PROVIDER = "openai"

# The per-provider default chat model, used by every task that doesn't pin its own
# (SUMMARY_MODEL / EXTRACTION_MODEL / QUERY_MODEL / STANCE_MODEL). The OpenAI
# default is the strongest general model; tune down per-task for cost if wanted.
...
# Part 2 (issue #13). The chat tasks (extraction, query, stance) default to the
# provider's default chat model (see `default_chat_model`) and are each tunable
# separately. The embedding model is pinned by ADR-0008 and stays on OpenAI.
